    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          ...
          {/* SEARCH BAR INPUT (Sesuai Mockup) */}
                  ...
                  {filteredProducts.length === 0 ? (
                    ...
                    filteredProducts.map((prod) => (
                      <tr key={prod.id} className="hover:bg-slate-50/40 transition-colors">
                        ...
                        {/* Badge Stok Hijau/Merah Lembut */}
                        <td className="p-4 text-center">
                          <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold ${
                            prod.stock > 20 ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'
                          }`}>
                            {prod.stock}
                          </span>
                        </td>
                        ...
                        {/* TOMBOL AKSI EDIT & HAPUS (MURNI SVG) */}
                        <td className="p-4">
                          <div className="flex justify-center items-center gap-3">
                            ...
                            {/* Tombol Hapus Tempat Sampah Merah Sesuai Mockup */}
                            <button 
                              onClick={() => handleDelete(prod.id, prod.title)} 
                              className="text-gray-400 hover:text-red-500 transition-colors p-1 hover:bg-red-550 rounded-lg"
                            >
                              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
